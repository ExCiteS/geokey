...
def meta(observation):
    username = get_user_name(observation.current_data.creator)
    date = format_date(observation.current_data.created_at)

    if observation.current_data.version == 1:
        ...
        return 'Created by %s on %s. Updated to version %s by %s on %s.' % (
            creator, create_date, observation.current_data.version,
            username, date)
