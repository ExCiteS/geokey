...
class FeaturetypeApiTest(ApiTest):
	...
	def test_featuretypesWithCreator(self):
		...
		self.assertContains(response, self.objectSerializer.serialize(self.publicFeatureType))
		
		response = self._get('eric', self.privateproject.id)
		self.assertEqual(response.status_code, 200)
		self.assertContains(response, self.objectSerializer.serialize(self.privateFeatureType))

		response = self._get('eric', self.inactiveproject.id)
		self.assertEqual(response.status_code, 200)
		self.assertContains(response, self.objectSerializer.serialize(self.inactiveFeatureType))
