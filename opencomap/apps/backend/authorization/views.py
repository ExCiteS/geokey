...
def get_list(user, project_id):
	"""
	Return a list of all views of the given project, that are accessible to the 
	user. The list may be empty if the user is not eligible to access any views.
	"""
	result = []
...
def create_usergroup(user, project_id, view_id, data, project=None):
	"""
	Creates a new user grpup the view for the given project and view, or raises 
...
def get_usergroup(user, project_id, view_id, group_id, project=None):
	"""
	Returns a new user grpup the view for the given project and view, or raises 
	`PermissionDenied` if the user is eligible to alter the project.
	"""
	view = project.view_set.get(pk=view_id)
...
def update_usergrpup(user, project_id, view_id, group_id, data, project=None):
	"""
	Updates new user group of the view for the given project and view, or raises 
	`PermissionDenied` if the user is eligible to alter the project.
	"""
	view = project.view_set.get(pk=view_id)
	group = view.viewgroup_set.get(pk=group_id)
...
@check_admin
def add_user_to_group(user, project_id, view_id, group_id, userToAdd, project=None):
	"""
	Adds a user to the user grpup of the view for the given project and view, or 
...
@check_admin
def remove_user_from_group(user, project_id, view_id, group_id, userToRemove, project=None):
	"""
	Removes a user from the user grpup of the view for the given project and 
	view, or raises `PermissionDenied` if the user is eligible to alter the project.
	"""
	view = project.view_set.get(pk=view_id)
	group = view.viewgroup_set.get(pk=group_id)
