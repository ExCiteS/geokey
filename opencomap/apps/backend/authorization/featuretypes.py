...
def get_list_field(user, project_id, featuretype_id):
	"""
	Returns a list of fiels for the given project and feature type. The list may 
	be empty if the user is not eligible to access any of the fields. 
	"""
...
@check_admin
def update_field(user, project_id, featuretype_id, field_id, data, project=None):
	"""
...
@check_admin
def add_lookup_value(user, project_id, featuretype_id, field_id, data, project=None):
	"""
	Adds a lookup value to a lookup new field for the project and feature type, 
...
@check_admin
def remove_lookup_value(user, project_id, featuretype_id, field_id, lookup_id, project=None):
	"""
	Removess a lookup value from the lookup new field for the project and 
	feature type, or raises `PermissionDenied` if the user is eligible to alter 
	the project.
	"""
	field = project.featuretype_set.get(pk=featuretype_id).getField(field_id)
