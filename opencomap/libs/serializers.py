...
field_registry = {
    ...
    NumericField: [
        'id', 'key', 'name', 'description', 'status', 'required',
        'minval', 'maxval'
    ],
    ...
    ViewGroup: [
        'id', 'name', 'description', 'can_admin', 'can_edit', 'can_view',
        'can_read', 'users'
    ],
}
...
class DataSerializer(serializers.get_serializer('python')):
    ...
    def serialize(self, queryset, compact=False, **options):
        ...
        if self.use_natural_keys:
            warnings.warn(
                "``use_natural_keys`` is deprecated; use"
                ...
                if field.serialize:
                    if field.rel is None:
                        if (fields_to_read is None
                                or field.attname in fields_to_read):
                            self.handle_field(obj, field)
                    else:
                        if (fields_to_read is None
                                or field.attname[:-3] in fields_to_read):
